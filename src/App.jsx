...
export default function GanttApp() {
  ...
  useEffect(()=>{
    getChannelId().then(async id => {
      ...
      try {
        ...
        try {
          const backupKey = "gantt_backup_" + (await getChannelId());
          const existingRaw = localStorage.getItem(backupKey);
          if (existingRaw) {
  ...
  useEffect(()=>{
    ...
    saveTimerRef.current = setTimeout(async () => {
      // SAFETY: detect suspicious data loss
      // If local items significantly fewer than last known server count, warn user
      const currentItemCount = items.length;
      const lastCount = lastServerCountRef.current;
      if (lastCount > 5 && currentItemCount < Math.floor(lastCount * 0.5)) {
        // 50% drop is suspicious — confirm with user
        const ok = window.confirm(
          `⚠️ 데이터 변경 경고\n\n` +
          `이전: ${lastCount}개 항목\n` +
          `현재: ${currentItemCount}개 항목\n\n` +
          `${lastCount - currentItemCount}개의 항목이 사라집니다. 정말 저장하시겠습니까?\n\n` +
          `(취소하면 새로고침으로 복구할 수 있습니다.)`
        );
        if (!ok) {
          setDbStatus("idle");
          return;
