...
export default function GanttApp() {
  ...
  const headerLayers = useMemo(() => {
    ...
    if (DAY_WIDTH < 3) {
      // Year above, month below. When extremely zoomed out, label only
      // every other month so the row stays legible instead of becoming a
      // wall of overlapping text.
      const thinMonths = DAY_WIDTH < 1.0; // ~ under 30px per month
      days.forEach(d => {
        ...
        // Always use the "N월" form — bare numbers are ambiguous. Thin to
        // odd months only when too narrow to show all twelve.
        const label = (thinMonths && m % 2 === 0) ? "" : `${m}월`;
