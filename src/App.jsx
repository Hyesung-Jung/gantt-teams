...
function DepArrows({ flatRows, viewStart, DAY_WIDTH, getRowTop, rowH }) {
  ...
  return (
    <svg style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:8,overflow:"visible"}}>
      ...
      {arrows.map(a => {
        // Orthogonal (right-angle) connector instead of a bezier curve.
        // Path: short horizontal stub out of the source end → vertical run
        // to the target row → horizontal into the target start. A small
        // elbow offset keeps the line from hugging the bars too tightly.
        // If the target starts to the left of the source end (overlapping
        // tasks), we still route with a fixed stub so the arrow stays
        // readable rather than doubling back on itself.
        const stub = 14;
        const midX = a.x2 - stub > a.x1 + stub
          ? a.x2 - stub               // normal case: elbow just before target
          : a.x1 + stub;              // overlapping case: elbow just after source
