...
function KanbanView({ items, onEdit, onDelete, openAdd, openAddSector, themeKey="dark", isMobile=false, collapsedSectors, toggleSector }) {
  ...
  const SECTOR_LABEL_W = isMobile ? 130 : 160;
  ...
  return (
    <div style={{flex:1, display:"flex", flexDirection:"column", background:T.bgDeep, overflow:"hidden"}}>
      <div style={{flex:1, overflowX:"auto", overflowY:"hidden", display:"flex", flexDirection:"column", WebkitOverflowScrolling:"touch"}}>
        <div style={{display:"flex", flexShrink:0, borderBottom:`2px solid ${T.border}`, background:T.bgHeader,
          ...
          <div style={{
            ...
            <button onClick={openAddSector} style={{
              ...
              background:themeKey==="dark"?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)",border:`1px solid ${T.border}`,
        ...
        <div style={{flex:1, overflowY:"auto", overflowX:"visible", minWidth: SECTOR_LABEL_W + KANBAN_COLS.length * COL_W, WebkitOverflowScrolling:"touch"}}>
          {sectors.map(sector => {
...
export default function GanttApp() {
  ...
  useEffect(()=>{
    ...
    if (window.microsoftTeams) {
      window.microsoftTeams.app.initialize().then(() => {
        window.microsoftTeams.app.getContext().then(ctx => {}).catch(() => {});
      }).catch(() => {});
    }
  }, []);
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        await sbSave(channelId, {
          ...
          projectName,
          uiState: { activeTab, zoomIdx, customDayWidth, showMilestones, themeKey, kanbanCollapsed }
        });
        setDbStatus("saved");
  ...
  }, [items, projectName, channelId, kanbanCollapsed, activeTab, zoomIdx, customDayWidth, showMilestones, themeKey]);
  ...
  // The other columns sum to ~192px (drag 26 + dot 6 + start 60 + end 60 + prog 40)
  // So baseNameWidth = leftPanelBase - 192
  const leftPanelBase = isMobile ? viewport.w : (isTablet ? 380 : 480);
  const otherColsWidth = 26 + 6 + 60 + 60 + 40 + 12; // grid + paddings
  const leftPanelWidth = isMobile
    ? viewport.w
    : (nameColWidth != null ? Math.max(leftPanelBase, nameColWidth + otherColsWidth) : leftPanelBase);
  const leftPanelOverlay = false; // we're using tab switching instead of overlay on mobile
  ...
  const saveForm=()=>{
    ...
    else { const{color:_c,_depSearch:__ds,...rest}=form; parsed={...rest,start:parseDate(form.start),end:parseDate(form.type==="milestone"?form.start:form.end),progress:form.type==="task"?(form.progress||0):0,deps:form.deps||[]}; }
      ...
      try {
        ...
        rows.forEach(row => {
          ...
          if (!sectorMap[sectorName]) {
            ...
            newItems.push({ id: sId, type: "sector", name: sectorName, color, collapsed: false, order: newItems.filter(i => i.type === "sector").length });
          }
          const itemId = ++maxId;
          const itemType = type === "마일스톤" ? "milestone" : "task";
          newItems.push({
            id: itemId, type: itemType,
            parentId: sectorMap[sectorName], name,
      ...
      {activeTab==="list" && (
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:T.bg}}>
          ...
          <div ref={leftPanelListRef} className="hide-scrollbar"
            ...
            <div style={{height:totalChartHeight,minHeight:"100%"}}>
              {flatRows.length === 0 && (
              ...
              <div style={{
                ...
                <div style={{flexShrink:0,borderBottom:`1px solid ${T.border}`,background:T.bgHeader}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:`26px 6px ${nameColWidth!=null?nameColWidth+"px":COL_NAME} ${COL_START} ${COL_END} ${COL_PROG}`,alignItems:"center",height:24,padding:"0 6px 0 4px"}}>
                    ...
                    <div style={{fontSize:10,fontWeight:600,color:T.textFaint,paddingLeft:4,position:"relative",height:"100%",display:"flex",alignItems:"center"}}>
                      ...
                      <div
                        onMouseDown={e=>{
                          e.preventDefault();
                          e.stopPropagation();
                          // Compute current rendered width if we're still in auto mode
                    ...
                    <div style={{fontSize:10,fontWeight:600,color:T.textFaint,textAlign:"center"}}>시작</div>
                    <div style={{fontSize:10,fontWeight:600,color:T.textFaint,textAlign:"center"}}>종료</div>
                    <div style={{fontSize:10,fontWeight:600,color:T.textFaint,textAlign:"center"}}>%</div>
                  </div>
                </div>
        ...
        <div style={{position:"fixed",inset:0,background:themeKey==="dark"?"rgba(0,0,0,0.78)":"rgba(0,0,0,0.4)",
          ...
          <div style={{background:T.bgCard,border:`1px solid ${T.border}`,
            ...
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              ...
              <F label="이름"><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} style={SB.inp} autoFocus/></F>
              {form.type!=="sector"&&(<>
            ...
            <div style={{display:"flex",gap:8,marginTop:20,justifyContent:"flex-end"}}>
              {editItem && (
                <button onClick={()=>{
                  if (window.confirm(`"${editItem.name}" 삭제하시겠습니까?`)) {
