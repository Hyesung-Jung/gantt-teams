...
const APP_VERSION = "1.0.8"; // 배포할때마다 올려주세요
...
export default function GanttApp() {
  ...
  const [customDayWidth, setCustomDayWidth] = useState(null); // null = use preset
  ...
  const zoomStateRef = useRef({}); // {startX, startY, scrollLeft, scrollTop}
  ...
  // -- Wheel zoom on chart (blocks scroll, does zoom) ----------------------
  useEffect(()=>{
    const el = chartRef.current;
    if (!el) return;
    const handler = e => {
      if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        ...
        const curDW = z.isFit
          ? Math.max(0.15, (el.clientWidth || 800) / (z.dataRangeDays || 100))
        ...
        if (z.isFit) setZoomIdx(1);
        if (newDW < 1) setZoomIdx(0);
        else if (newDW < 5) setZoomIdx(1);
        else if (newDW < 18) setZoomIdx(2);
        else setZoomIdx(3);
      }
    };
    ...
    reader.onload = (e) => {
      try {
        ...
        rows.forEach(row => {
          ...
          // row[5] = days (skip, calculated from start/end)
  ...
  if (!items || !Array.isArray(items)) return <div style={{padding:40,color:"#f87171"}}>Data error. Please refresh.</div>;
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
              ...
              <div ref={chartRef}
                ...
                <div
                  ...
                  {days.map((d,i)=>{
                    if (DAY_WIDTH < 5) {
                      // Show vertical lines only at Jan 1st and Jul 1st (half-year)
                    ...
                    // Normal modes: show weekend shading
                  ...
                  {flatRows.map(({item},rowIdx)=>{
                    ...
                    if (item.type==="sector") {
                      ...
                      return <div key={item.id} style={{position:"absolute",top:top+ROW_HEIGHT/2-Math.round(ROW_HEIGHT*0.1),left:l,width:w,height:Math.round(ROW_HEIGHT*0.2),background:themeKey==="light"?hexAlpha(item.color,0.12):hexAlpha(item.color,0.18),borderRadius:4,border:`1px solid ${hexAlpha(item.color,themeKey==="light"?0.25:0.35)}`,pointerEvents:"none"}}/>;
                    }
                    if (item.type==="milestone") {
                      ...
                      return (
                        <div key={item.id} title={item.name+(item.link?" ":"")}
                          ...
                          {DAY_WIDTH>=8&&<div style={{position:"absolute",top:sz*2+2,left:"50%",transform:"translateX(-50%)",fontSize:9,color:T.textSub,whiteSpace:"nowrap",fontWeight:600}}>{item.name}</div>}
                        </div>
                      );
                    }
                    if (!item.start) return null;
