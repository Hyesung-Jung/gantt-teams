...
async function sbSave(channelId, data) {
  ...
  // Try upsert first
  const res = await fetch(`${SUPABASE_URL}/rest/v1/gantt_data`, {
    method: "POST",
  ...
  if (!res.ok) {
    ...
    const res2 = await fetch(`${SUPABASE_URL}/rest/v1/gantt_data?id=eq.${channelId}`, {
      method: "PATCH",
...
export default function GanttApp() {
      ...
      try {
        await sbSave(channelId, {
            ...
            uiState: {
              ...
              showMilestones,
              themeKey,
              collapsedSectors: items.filter(i=>i.type==="sector"&&i.collapsed).map(i=>i.id),
            }
          });
        setDbStatus("saved");
        ...
        setTimeout(async () => {
          try {
            await sbSave(channelId, {
              ...
              projectName,
              uiState: {
                activeTab, zoomIdx, showMilestones, themeKey,
                collapsedSectors: items.filter(i=>i.type==="sector"&&i.collapsed).map(i=>i.id),
              }
            });
            setDbStatus("saved");
  ...
  const fitRange = useMemo(() => {
    ...
    minD.setDate(minD.getDate() - 3); // 3일 여백
    maxD.setDate(maxD.getDate() + 3);
    const totalD = Math.max(dateDiff(minD, maxD) + 1, 7);
    // chartRef width for dayWidth calculation
    const chartW = (chartRef.current?.clientWidth || 800) - 2;
    const dw = Math.max(4, Math.floor(chartW / totalD));
