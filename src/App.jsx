...
async function getChannelId() {
  ...
  if (urlToken) {
    ...
    if (insideTeams) {
      try {
        ...
        // If we couldn't get context (timeout) but we are inside Teams,
        // allow but the channel_id from the mapping table is the source of truth.
      } catch(e) {
        // SDK error inside Teams: allow but rely on token mapping.
      }
    } else {
      // No Teams SDK at all → page was opened directly in a browser.
      // For a token URL, we *could* still serve the data, but that defeats
      // the whole point of this hardening. Block it.
      return BLOCKED;
    }
  ...
  if (urlCh && urlCh !== "default") {
    if (!insideTeams) {
      // Same reasoning as above — direct browser access without Teams = block.
      return BLOCKED;
    }
    // We could enforce a Teams context match here too, but legacy URLs
    // explicitly opted out of this hardening. Just trust the URL.
