...
function KanbanView({ items, onEdit, onDelete, openAdd, openAddSector }) {
  ...
  const getCards = (sectorId, statusKey) =>
    items.filter(i => i.parentId === sectorId && i.type !== "sector" && getKStatus(i) === statusKey)
      .slice().sort((a,b) => (a.order||0)-(b.order||0));
  const moveCard = (cardId, newStatus) => {
    const pm = { todo:0, inprogress:30, review:75, done:100 };
  ...
  return (
    <div style={{flex:1, display:"flex", flexDirection:"column", background:"#0b0e17", overflow:"hidden"}}>
      ...
      <div style={{display:"flex", flexShrink:0, borderBottom:"2px solid #1e2535", background:"#0d111a"}}>
      ...
      <div style={{flex:1, overflowY:"auto", overflowX:"auto"}}>
        {sectors.map(sector => {
          ...
          const totalTasks = items.filter(i=>i.parentId===sector.id&&i.type!=="sector").length;
          const doneTasks  = items.filter(i=>i.parentId===sector.id&&i.type!=="sector"&&i.progress===100).length;
...
export default function GanttApp() {
  ...
  const totalDays = zoom.totalDays;
  ...
  const days = useMemo(()=>{
    const arr=[]; const cur=new Date(viewStart);
  ...
  const todayX=dateDiff(viewStart,today)*DAY_WIDTH+DAY_WIDTH/2;
  ...
  function ToolbarContent({vertical=false}) {
    return (
      <div style={{display:"flex",flexDirection:vertical?"column":"row",gap:vertical?6:5,alignItems:vertical?"stretch":"center",padding:vertical?"8px 10px":0}}>
        ...
        <button onClick={()=>setViewStart(addDays(today,-7))} style={SB.today}>오늘</button>
        <button onClick={()=>setViewStart(addDays(viewStart,Math.max(1,Math.floor(totalDays/4))))} style={SB.nav}>
          다음
                ...
                <div style={{width:totalW,minHeight:totalChartHeight,position:"relative"}}>
                  ...
                  {flatRows.map(({item},rowIdx)=>{
                    ...
                    if (item.type==="sector") {
                      ...
                      const l=dateDiff(viewStart,ms)*DAY_WIDTH;
                    ...
                    if (item.type==="milestone") {
                      ...
                      const cx=(dateDiff(viewStart,item.start)+0.5)*DAY_WIDTH;
                    ...
                    const left=dateDiff(viewStart,item.start)*DAY_WIDTH;
                            ...
                            {deps.map(depId => {
                              ...
                              return (
                                <span key={depId} style={{display:"flex",alignItems:"center",gap:5,
                                  ...
                                  {dep.name}
                                  <span onClick={()=>toggle(depId)} style={{cursor:"pointer",color:"#f87171",fontWeight:700,fontSize:14,lineHeight:1}}>×</span>
                                </span>
                              );
                            })}
                          </div>
