...
const APP_VERSION = "1.0.5"; // 배포할때마다 올려주세요
...
const ZOOM_LEVELS = [
  { key:"fit",     label:"전체", dayWidth:0  },
  { key:"year",    label:"연",   dayWidth:3  },
  { key:"month",   label:"월",   dayWidth:12 },
  { key:"day",     label:"일",   dayWidth:28 },
];
    ...
    <>
      <div onMouseEnter={()=>onHover(item.id)} onMouseLeave={()=>onHover(null)}
        ...
        <div style={{textAlign:"center",fontSize:10,fontWeight:700,display:"flex",justifyContent:"center",alignItems:"center"}}>
          {item.type==="task" ? (
            <span style={{
              background:item.progress===100?"#10b981":item.progress>0?"#6366f1":"transparent",
              color:item.progress>0?"#fff":T.textDim,
              borderRadius:10,padding:item.progress>0?"2px 7px":"0",
              fontSize:10,fontWeight:700,
              border:item.progress===0?("1px solid "+T.border):"none",
              minWidth:item.progress===0?30:"auto",
            }}>{item.progress}%</span>
          ) : ""}
        </div>
      </div>
...
export default function GanttApp() {
  ...
  const [chartPan, setChartPan] = useState(null); // {startX, startY, scrollLeft, scrollTop}
  ...
  const dataRange = useMemo(() => {
    ...
    minD.setDate(minD.getDate() - 14); // 2주 여백
  ...
  const DAY_WIDTH = isFit ? Math.max(1, chartW / dataRangeDays) : zoom.dayWidth;
  ...
  const headerGroups = useMemo(()=>{
    const g=[];
    days.forEach((d,i)=>{
      const label = (zoom.key==="year" || zoom.key==="fit") ? `${d.getFullYear()}`
        : zoom.key==="month" ? `${d.getFullYear()}년 ${d.getMonth()+1}월`
          ...
          <div style={{display:"flex",flex:1,overflow:"hidden",position:"relative"}}>
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
                    ...
                    {days.map((d,i)=>{
                      ...
                      return (
                        <div key={i} style={{width:DAY_WIDTH,flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                          ...
                          {zoom.key==="day"&&DAY_WIDTH>=14&&<span style={{fontSize:DAY_WIDTH<14?7:9,fontWeight:isT?700:400,color:isT?"#818cf8":T.textDim}}>
              ...
              <div ref={chartRef}
                ...
                <div
                  onMouseDown={e=>{
                    // Only pan when clicking on empty chart area (not bars/milestones)
                    if (e.button !== 0) return;
                    const tag = e.target.tagName;
                    if (tag === "BUTTON" || tag === "INPUT" || tag === "SELECT") return;
                    // Check if clicked on a bar (bars have zIndex:6)
                    let el = e.target;
                  ...
                  {days.map((d,i)=>(d.getDay()===0||d.getDay()===6)&&(
                    <div key={i} style={{position:"absolute",left:i*DAY_WIDTH,top:0,width:DAY_WIDTH,height:"100%",background:themeKey==="dark"?"rgba(255,255,255,0.012)":"rgba(0,0,30,0.025)",pointerEvents:"none"}}/>
                  ))}
                  {flatRows.map((_,i)=>{
                  ...
                  {flatRows.map(({item},rowIdx)=>{
                    ...
                    if (item.type==="sector") {
                      ...
                      return <div key={item.id} style={{position:"absolute",top:top+ROW_HEIGHT/2-5,left:l,width:w,height:10,background:themeKey==="light"?hexAlpha(item.color,0.12):hexAlpha(item.color,0.18),borderRadius:4,border:`1px solid ${hexAlpha(item.color,themeKey==="light"?0.25:0.35)}`,pointerEvents:"none"}}/>;
                    }
