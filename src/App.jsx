...
export default function GanttApp() {
  ...
  // Two-layer adaptive timeline header. The top row is always the year;
  // the bottom row changes with zoom level so the user always sees the
  // appropriate sub-unit (분기/월/일) instead of only seeing "2024년 5월"
  // labels repeated across years with no sub-division.
  //
  //   DAY_WIDTH zone        | top         | bottom
  //   ──────────────────────┼─────────────┼──────────────────
  //   < 3   (fit / very wide) | 연          | Q1 Q2 Q3 Q4
  //   < 18  (year/month)     | 연          | 1월 ... 12월
  //   ≥ 18  (day)            | 연 + 월     | 일자 (in days row)
  //
  const headerLayers = useMemo(() => {
    // High zoom: keep the existing single-line "연-월" header and let
    // the per-day row below it show individual dates.
    if (DAY_WIDTH >= 18) {
      const top = [];
      days.forEach(d => {
    ...
    // Low zoom: year on top, sub-units below.
    const topGroups = [];
    days.forEach(d => {
      const year = d.getFullYear();
      const last = topGroups[topGroups.length-1];
      if (!last || last.year !== year) topGroups.push({label: String(year), year, count: 1});
      else last.count++;
    });
    const bottomGroups = [];
    if (DAY_WIDTH < 3) {
      // Very zoomed out → quarters. Keyed by year + quarter so two
      // adjacent same-quarters from different years don't merge.
      days.forEach(d => {
        const q = Math.floor(d.getMonth() / 3) + 1;
        const year = d.getFullYear();
        const last = bottomGroups[bottomGroups.length-1];
        if (!last || last.q !== q || last.year !== year) {
      ...
      // Mid zoom → months. Compact label when very narrow.
      days.forEach(d => {
        const m = d.getMonth() + 1;
        const year = d.getFullYear();
        const label = DAY_WIDTH < 6 ? String(m) : `${m}월`;
        const last = bottomGroups[bottomGroups.length-1];
        if (!last || last.m !== m || last.year !== year) {
          bottomGroups.push({label, m, year, count: 1});
        } else {
          last.count++;
        }
      });
  ...
  const importFromXLSX = async (file) => {
    try {
      ...
      const newItems = [];
      rows.forEach(row => {
        let categoryName, sectorName, type, name, startStr, endStr, progress, link, depsStr, notesStr;
        if (hasCategory) {
  ...
  const exportToExcel=async()=>{
    try {
      const XLSX = await loadXLSX();
      const categories=items.filter(i=>i.type==="category").slice().sort((a,b)=>(a.order??0)-(b.order??0));
      const rows=[["대분류","섹터","유형","작업명","시작일","종료일","일수","진행률(%)","링크","의존작업","메모"]];
      categories.forEach(category=>{
